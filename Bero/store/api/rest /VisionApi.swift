...
class VissionApi :Rest{
    func post(img:UIImage, thumbImg:UIImage, action:ApiAction, completion: @escaping (ApiContentResponse<DetectData>) -> Void, error: ((_ e:Error) -> Void)? = nil){
        fetch(route: VissionImagesApiRoute(method: .post, action: action),
           constructingBlock:{ data in
            ...
            if let value = thumbImg.jpegData(compressionQuality: 1.0) {
                data.append(file: value,name: "smallContents",fileName: "visionImageThumb.jpg",mimeType:"image/jpeg")
            }
