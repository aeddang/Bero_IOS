...
struct WalkTopInfo: PageComponent{
        ...
        .onReceive(self.dataProvider.$result){ res in
            ...
            case .updateAlbumPicture(let pictureId, _, let isExpose):
                if let expose = isExpose {
                    self.mission.isExpose = expose
                    self.updated(pictureId, isExpose:expose)
                }
            default : break
            }
        }
    ...
    private func updated(_ id:Int, isExpose:Bool){
        if self.mission.walkPath?.picture?.pictureId == id {
           self.appSceneObserver.event = .toast(isExpose ? String.alert.exposed : String.alert.unExposed)
        }
    }
}
