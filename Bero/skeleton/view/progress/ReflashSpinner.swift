    ...
    var body: some View {
        VStack{
            Image(Asset.icon.refresh)
                ...
                .colorMultiply(self.progress > self.progressMax ? Color.brand.primary : Color.app.grey100)
            if text != nil {
                Text(text!)
                .modifier(LightTextStyle(size: Font.size.light, color: Color.app.grey100))
            }
        }
