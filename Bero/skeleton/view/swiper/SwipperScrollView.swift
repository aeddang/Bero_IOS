...
struct SwipperScrollView<Content> : PageView, Swipper  where Content: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .highPriorityGesture(
                DragGesture(minimumDistance: SwipperView.MIN_DRAG_RANGE, coordinateSpace: self.coordinateSpace)
                .onChanged({ value in
                    self.isUserSwiping = true
                    let willOffset = self.getDragOffset(value: value, geometry: geometry)
                    if let pull = self.usePull {
                        if pull == .horizontal {
                            if willOffset > SwipperView.PULL_RANGE || self.viewModel.status == .pull {
                                self.viewModel.event = .pull(willOffset/4.0)
                                self.viewModel.status = .pull
                                self.progress = self.progressMax - Double(willOffset-SwipperView.PULL_RANGE)
                            } else {
                            ...
                            if pullOffset > SwipperView.PULL_RANGE || self.viewModel.status == .pull {
                                self.viewModel.event = .pull(pullOffset/2.0)
                                self.viewModel.status = .pull
                                self.progress = self.progressMax - Double(pullOffset-SwipperView.PULL_RANGE)
                            } else {
                                self.viewModel.status = .move
                                self.offset = willOffset
                            }
                ...
                .onEnded({ value in
                    ...
                    case .pull :
                        ...
                        if willPullOffset > SwipperView.PULL_COMPLETE_RANGE{
                    ...
                    self.reset(idx: self.getWillIndex(value: value, maxIdx: self.count) )
                })
            )
            .gesture(
            ...
            .onReceive(self.viewModel.$request){ evt in
                ...
                case .jump(let idx) :
                    ...
                    self.viewModel.index = idx
                case .prev:
                    let willIdx = self.index == 0 ? self.count : self.index - 1
                ...
                case .next:
                    let willIdx = self.index >= self.count ? 0 : self.index + 1
                    self.offset = CGFloat(willIdx) * -geometry.size.width
                    self.viewModel.status = .move
                    self.viewModel.request = .drag(self.offset)
                    self.isUserSwiping = true
            ...
            .onAppear(){
                ...
                self.progressMax = Double(SwipperView.PULL_COMPLETE_RANGE - SwipperView.PULL_RANGE)
